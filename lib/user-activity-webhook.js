...
module.exports = function userActivityWebhook (config) {
    ...
    const middleware = function userActivityWebhookMiddleware (req, res, next) {
        ...
        case 'GET':
            if (req.query.crc_token) {
                debuglog('CRC token request received');
                return res.json({
        ...
        case 'POST':
            res.sendStatus(200);
            debuglog('User activity payload received : ', req.body);
    ...
    middleware.getSubscriptions = function (args = {}) {
        ...
        .then(options => {
            ...
            return sendApiRequest(options)
            .then(function (response) {
                if (response.rateLimit) middleware.getSubscriptions.rateLimit = response.rateLimit;
                return response.body.subscriptions;
            })
            .catch(function (e) {
                if (e.rateLimit) {
                    middleware.getSubscriptions.rateLimit = e.rateLimit;
                    delete e.rateLimit;
                }
                throw e;
            });
